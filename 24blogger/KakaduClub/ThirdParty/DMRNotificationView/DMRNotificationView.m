...
static CGFloat kNotificationViewVerticalInset = 14.0;                   // Top and bottom inset
...
-(id)initWithTitle:(NSString *)title subTitle:(NSString *)subTitle targetView:(UIView *)view
{
    ...
    {        
        ...
        [self setTintColor:[UIColor colorWithRed:24.0f/255.0f green:111.0f/255.0f blue:181.0f/255.0f alpha:1.0f]];
...
-(void)drawRect:(CGRect)rect
{
    ...
    CGContextSetShadowWithColor(ref, CGSizeMake(0.0, 1.0), kNotificationViewShadowOffset, [UIColor colorWithWhite:0.0 alpha:0.5].CGColor);
...
-(void)showAnimated:(BOOL)animated
{
    ...
    [self setCenter:CGPointMake(self.center.x, self.center.y-self.bounds.size.height + 22.0f)];
...
-(void)dismissAnimated:(BOOL)animated
{
    if (animated)
    {
        [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationCurveEaseInOut animations:^{
            [self setCenter:CGPointMake(self.center.x, -self.bounds.size.height + 22.0f)];
...
-(void)showAnimated:(BOOL)animated deltaPosition:(float)deltaPosition
{
    [self dismissAnimated:NO];
    ...
    CGPoint animateToCenter = self.center;
    [self setCenter:CGPointMake(self.center.x, self.center.y-self.bounds.size.height + deltaPosition)];
    [_targetView addSubview:self];
...
-(void)dismissAnimated:(BOOL)animated  deltaPosition:(float)deltaPosition
{
    if (animated)
    {
        [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationCurveEaseInOut animations:^{
            [self setCenter:CGPointMake(self.center.x, -self.bounds.size.height + deltaPosition)];
        } completion:^(BOOL finished) {
            [self removeFromSuperview];
        }];
    }
...
-(UIFont *)titleFont
{
    ...
    return [UIFont fontWithName:KAKADU_FONT_NORMAL size:14.0];
}
...
-(UIFont *)subTitleFont
{
    ...
    return [UIFont fontWithName:KAKADU_FONT_NORMAL size:13.0];
}
