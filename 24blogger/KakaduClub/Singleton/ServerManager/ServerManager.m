...
- (void)pushNotificationPutWithDeviceToken:(NSString *)deviceToken
                    onSuccess:(void(^)(NSDictionary *dictionary)) success
                    onFailure:(void(^)(NSError *error,NSInteger statusCode)) failure {
    [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:YES];
    self.requestOperationManager.responseSerializer = [AFJSONResponseSerializer serializer];
    [self.requestOperationManager GET:[NSString stringWithFormat:@"zaki-push-notification/put/token/%@", KAKADU_URL]
                           parameters:nil
                              success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)pushNotificationPutWithDeviceUDIDToken:(NSString *)deviceUDIDToken {
    NSLog(@"deviceUDIDToken = %@", deviceUDIDToken);
    [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:YES];
